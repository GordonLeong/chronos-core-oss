...
export function UniverseFormPanel({
  ...
  return (
    <section className="rounded border p-3 space-y-3 text-sm">
      ...
      {universeSuccess && (
        <p className="rounded border border-green-300 bg-green-50 p-2 text-green-700">{universeSuccess}</p>
      )}
      {universeError && (
        <p className="rounded border border-red-300 bg-red-50 p-2 text-red-700">{universeError}</p>
      )}
      ...
      {tickerSuccess && (
        <p className="rounded border border-green-300 bg-green-50 p-2 text-green-700">{tickerSuccess}</p>
      )}
      {tickerError && (
        <p className="rounded border border-red-300 bg-red-50 p-2 text-red-700">{tickerError}</p>
      )}
    </section>
  );
}